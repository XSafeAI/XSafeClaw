...
@pytest.mark.asyncio
async def test_dashboard_resolves_nanobot_placeholder_from_session_model(db, monkeypatch):
    monkeypatch.setattr(
        stats_routes,
        "_read_openclaw_config",
        lambda: {
    ...
    async with get_db_context() as session:
        session.add(
            Session(
                session_id="nanobot::nanobot-default::chat-1",
                platform="nanobot",
    ...
    assert data["cost"] == pytest.approx((1000 * 0.14 + 2000 * 0.28) / 1_000_000, abs=1e-9)
    assert data["costUnknownTokens"] == 0
    assert data["costBreakdown"][0]["priced"] is True
    assert data["costBreakdown"][0]["costMethod"] == "direct"
    assert data["costBreakdown"][0]["resolvedProvider"] == "deepseek"
    assert data["costBreakdown"][0]["resolvedModelId"] == "deepseek-v4-flash"
