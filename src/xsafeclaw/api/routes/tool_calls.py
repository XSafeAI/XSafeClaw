...
async def list_tool_calls(
    ...
    base_stmt = apply_runtime_filters(select(ToolCall), ToolCall, platform=platform, instance_id=instance_id)
    if tool_name:
    ...
    stmt = base_stmt.order_by(ToolCall.started_at.desc().nullslast(), ToolCall.created_at.desc()).offset(offset).limit(page_size)
    result = await db.execute(stmt)
    tool_calls = result.scalars().all()

    return ToolCallListResponse(
        tool_calls=[ToolCallResponse.model_validate(tc, from_attributes=True) for tc in tool_calls],
        total=total,
        page=page,
        page_size=page_size,
    )
...
async def get_tool_call(
    ...
    return ToolCallResponse.model_validate(tool_call, from_attributes=True)
...
async def get_tool_call_stats(
    ...
    stmt = select(
        ...
        func.sum(case((ToolCall.is_error.is_(False), 1), else_=0)).label("successful_calls"),
        func.sum(case((ToolCall.is_error.is_(True), 1), else_=0)).label("failed_calls"),
        func.avg(ToolCall.duration_seconds).label("avg_duration"),
    )
    stmt = apply_runtime_filters(stmt, ToolCall, platform=platform, instance_id=instance_id)
    stmt = stmt.group_by(ToolCall.tool_name).order_by(func.count(ToolCall.id).desc())
