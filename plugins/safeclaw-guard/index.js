...
export default function register(api) {
    ...
    const baseUrl = (cfg.safeclawUrl ??
        process.env.SAFECLAW_URL ??
        "http://localhost:6874").replace(/\/$/, "");
    ...
    api.on("before_tool_call", async (event, ctx) => {
        try {
            ...
            clearTimeout(timer);
            if (!resp.ok) {
                api.logger.warn?.(`safeclaw-guard: tool-check returned ${resp.status}`);
                if (failOpenOnGuardError)
                    return;
                return {
                    block: true,
            ...
            api.logger.warn?.(`safeclaw-guard: hook error: ${err?.message || err}`);
            if (failOpenOnGuardError)
                return;
            return {
                block: true,
